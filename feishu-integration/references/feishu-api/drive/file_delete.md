> API: https://open.feishu.cn/document/server-docs/docs/drive-v1/file/delete
...
接口频率限制 |   
支持的应用类型 |  自建应用 商店应用  
权限要求 开启任一权限即可 |  查看、评论、编辑和管理云空间中所有文件 删除云空间文件夹和云文档  
### 请求头
名称 | 类型 | 必填 | 描述  
---|---|---|---  
...
### 路径参数  
file_token | string |  需要删除的文件或文件夹 token。 了解如何获取文件 token，参考[文件概述](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/drive-v1/file/file-overview)。 了解如何获取文件夹 token，参考[文件夹概述](https://open.feishu.cn/document/ukTMukTMukTM/ugTNzUjL4UzM14CO1MTN/folder-overview)。 **示例值** ："boxcnrHpsg1QDqXAAAyachabcef"  
### 查询参数  
type | string |  被删除文件的类型 **示例值** ："file" **可选值有** ：
  * `file`：文件类型
  * `docx`：新版文档类型
  * `bitable`：多维表格类型
  * `folder`：文件夹类型
