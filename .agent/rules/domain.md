...
| CLI 包提取 | `packages/{brainstorm,swe-team,dispatch}-cli/` 各自独立 package.json + esbuild ESM 打包；产物输出到对应 skill 的 `scripts/` | skill 目录纯净（无 node_modules / package.json）；打包产物零运行时依赖；与 feishu-ts → feishu-cli.cjs 模式一致 |
