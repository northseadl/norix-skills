...
| SWE Team Workshop 架构 | Hub(HTTP Server) + Board(board.json) + MeetingRoom(meeting.jsonl) + ws.mjs(CLI)；Agent 通过 Codex/Claude/OpenCode SDK 生成；port 文件发现机制 | Supervisor 只需 shell 能力（兼容 Antigravity/Cloud Code）；状态全持久化，Hub 重启可恢复 |
| SWE Team 三引擎适配 | Codex=thread resume(streaming)；Claude=new session per turn；OpenCode=`createOpencodeClient` 连接已运行实例 + `promptAsync` 触发推理 + 轮询 `messages()` 等待 `finish=stop` | OpenCode: `prompt()` 在第一步后就返回不等待 agentic loop；`session.status()` 始终返回 `{}`；正确方式是 `promptAsync` + 消息轮询直到 `lastAssistantMsg.info.finish === "stop"` |
| SWE Team 持久会话模型 | Agent session 跨任务持续（threadId 不清空）；Codex=thread resume；Claude=new session with full context；OpenCode=同 sessionId 多轮 prompt | 角色上下文保留在 session 历史中；resume prompt 只需增量信息 |
