...
├── agent-task-orchestration/   # 编排专用引擎 v0.2.0（原子写入·AbortController·事件环缓冲·崩溃安全）
...
| NX1 凭据加密 | PBKDF2-SHA256 流密码 + HMAC + 平台 HWID 持久化种子派生密钥 | 零依赖·跨平台·磁盘从不出现明文·`~/.agents/.machine-seed` 首次初始化后不变 |
